  ...
  inquirer.prompt(questions).then(answers => {
    ...
    if (fs.existsSync(`android`)) {
      ...
      shell.ls('-Rl', 'android').forEach(entry => {
        if (entry.isFile() && !entry.name.match(/.(jpg|jpeg|png|gif|jar)$/i)) {
    ...
    if (fs.existsSync(`ios`)) {
      ...
      if (fs.existsSync(`ios/Bonjour.xcworkspace`)) {
        shell.mv(`ios/Bonjour.xcworkspace`, `ios/${projectName}.xcworkspace`);
      }
    }

    // Replace variable values in all localPath files,
    // except templates/ios and templates/android folders
    shell.ls('-Rl', ['LICENSE', 'package.json']).forEach(entry => {
      if (entry.isFile()) {
